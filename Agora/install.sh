...
# === Effacement de l'affichage des retours stdout === #
exec 1>/dev/null
...
if [[ $DL_FAIL -eq 1 ]]; then
    ...
    echo "Erreur lors du téléchargement des installateurs." >&2
    exit 1
fi
...
for mod in "${MODULES[@]}"; do
    ...
    if [[ -f "$SCRIPT" ]]; then
        ...
        if bash "$SCRIPT" >/dev/null 2>&1; then
            ...
            # Interruption stricte si le sous-module Cerbere échoue
            if [[ "$mod" == "Cerbere" ]]; then
                echo "Échec critique : l'installation du sous-module Cerbere a échoué. Arrêt du script." >&2
                exit 1
            fi
        fi
    else
        ...
        if [[ "$mod" == "Cerbere" ]]; then
            log "Lancement de Cerbere : FAILED (script introuvable)"
            echo "Échec critique : le script d'installation de Cerbere est introuvable. Arrêt du script." >&2
            exit 1
        fi
    fi
done
...
# === Chargement des identifiants (générés par Cerbere) === #
CREDENTIALS_FILE="$BASE_DIR/credentials.sh"
if [[ -f "$CREDENTIALS_FILE" ]]; then
    # shellcheck source=/dev/null
    source "$CREDENTIALS_FILE"
fi

# Valeurs par défaut si non définies dans credentials.sh
PathLamp="${PathLamp:-/var/www/html}"
PathPmTiles="${PathPmTiles:-/var/www/html/europe.pmtiles}"
PortLamp="${PortLamp:-80}"

# === Installation du docker LAMP === #
log "Installation du Docker LAMP : PENDING"

# Création des dossiers locaux s'ils n'existent pas pour éviter une erreur Docker
mkdir -p "$PathLamp"
touch "$PathPmTiles" 2>/dev/null || true
